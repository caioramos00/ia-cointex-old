...
const { ensureEstado } = require('./bot.js');  // Assumindo que stateManager.js será criado em breve; ajuste se necessário
...
async function sendImage(contato, urlOrItems, captionOrOpts, opts = {}) {
    ...
    opts = { delayBetweenMs: [12000, 16000], ...opts };
    await extraGlobalDelay();
...
async function sendManychatWaFlow(contato, flowNs, dataOpt = {}) {
    ...
    let subscriberId = await resolveManychatSubscriberId(contato, mod, settings);
    if (!subscriberId) {
