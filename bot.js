...
async function retomarEnvio(contato) {
  ...
  if (
    st2 &&
    st2.etapa === 'instruções' &&
    st2.credenciais &&
    st2.credenciais.username &&
    st2.credenciais.password &&
    st2.credenciais.link &&
    !st2.credenciaisEntregues
  ) {
    const mensagensAcesso = [
      'vamos começar, beleza?',
      'não manda áudio e só responde com o que eu pedir',
      'USUÁRIO: ',
      String(st2.credenciais.username || '').trim(),
      'SENHA: ',
      String(st2.credenciais.password || '').trim(),
      String(st2.credenciais.link || '').trim(),
      'me avisa assim que vc entrar. manda só "ENTREI" pra agilizar'
    ];
    await enviarLinhaPorLinha(contato, mensagensAcesso.join('\n'));
    const concluiu = !st2.seqLines;
    st2.credenciaisEntregues = !!concluiu;
    st2.etapa = 'acesso';
    st2.tentativasAcesso = 0;
    st2.mensagensDesdeSolicitacao = [];
    await atualizarContato(contato, 'Sim', 'acesso', 'credenciais enviadas (retomada)');
  }
      ...
      if (estado.instrucoesCompletas && mensagensPacote.length > 0) {
        ...
        if (tipoAceite.includes('ACEITE') && !estado.mensagemDelayEnviada) {
          if (
            ...
            estado.tentativasAcesso = 0;
            estado.mensagensDesdeSolicitacao = [];
            await atualizarContato(contato, 'Sim', 'acesso', 'credenciais enviadas (sem espera)');
            return; // já avançou, não manda "5 minutinhos..."
          }

          // fallback: não tem credenciais ainda → mantém o comportamento atual de "5 minutinhos..."
          if (!estado.mensagemDelayEnviada) {
