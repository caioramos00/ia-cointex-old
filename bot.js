...
const { escreverArquivoComLock, delay, gerarBlocoInstrucoes, mensagensIntrodutorias, checklistVariacoes, mensagensPosChecklist, respostasNaoConfirmadoAcesso, respostasNaoConfirmadoConfirmacao, respostasDuvidasComuns, gerarSenhaAleatoria } = require('./utils');
const { gerarResposta, criarUsuarioDjango, sendMessage } = require('./integrations');
...
// Bot functions
const inicializarEstado = (contato) => {
...
const atualizarContato = async (contato, conversou, etapa_atual, mensagem = null, temMidia = false) => {
  try {
    const GRUPOS_CONTATOS_FILE = './grupos_e_contatos.json';
...
const enviarLinhaPorLinha = async (to, texto) => {
  ...
  estado.enviandoMensagens = false;
  if (estado.mensagensPendentes.length > 0) {
    console.log(`[${to}] Mensagens acumuladas durante o envio, processando agora`);
    try {
      await processarMensagensPendentes(to);
    } catch (error) {
      console.error(`[${to}] Erro ao processar mensagens pendentes após envio: ${error.message}`);
    }
  }
  console.log(`[${to}] Envio concluído: "${texto}"`);
};

const processarMensagensPendentes = async (contato) => {
  try {
    const estado = estadoContatos[contato];
    const estadoSemTimeout = Object.assign({}, estado, { acompanhamentoTimeout: estado && estado.acompanhamentoTimeout ? '[Timeout]' : null, delayTimeout: estado && estado.delayTimeout ? '[Timeout]' : null });
    ...
    if (estado.inMultiSend) {
      console.log("[" + contato + "] Delayed processing during multi send");
      return;
    }

    // Clear old timeouts if in wrong stage
    if (estado.etapa !== 'instruções' && estado.delayTimeout) {
      clearTimeout(estado.delayTimeout);
      estado.delayTimeout = null;
    }
    if (estado.etapa !== 'validacao' && estado.acompanhamentoTimeout) {
      clearTimeout(estado.acompanhamentoTimeout);
    ...
    if (estado.etapa === 'instruções') {
      ...
      if (estado.instrucoesCompletas && mensagensPacote.length > 0) {
        ...
        if (tipoAceite.includes('aceite') && !estado.mensagemDelayEnviada) {
          ...
          estado.delayTimeout = setTimeout(async () => {
            estado.delayTimeout = null;
