...
async function sendImage({ subscriberId, imageUrl, caption }, settings, opts = {}) {
  ...
  if (caption) imgMsg.image.caption = String(caption).slice(0, 1024);
  ...
  let r1 = await call('/fb/sending/sendContent', payloadImg, token, 'sendContent:image');

  if (r1.status >= 400 || r1.data?.status === 'error') {
    r1 = await call('/whatsapp/sending/sendContent', payloadImg, token, 'sendContent:image-whatsapp-fallback');
  }
  ...
  if (shouldAlsoSendAsFile) {
    const docMsg = {
      type: 'document',
      document: { link: finalUrl, filename }
    };
    if (caption) docMsg.document.caption = String(caption).slice(0, 1024);

    const payloadFile = {
      subscriber_id: subscriberId,
      data: { version: 'v2', content: { type: 'whatsapp', messages: [docMsg] } },
    };

    let r2 = await call('/fb/sending/sendContent', payloadFile, token, 'sendContent:document-fallback');
    if (r2.status >= 400 || r2.data?.status === 'error') {
      r2 = await call('/whatsapp/sending/sendContent', payloadFile, token, 'sendContent:document-whatsapp-fallback');
    }
  }
