...
function contentV2Text(textOrLines) {
  ...
  return {
    version: 'v2',
    content: { type: 'whatsapp', messages }, // <- canal correto via DC v2
  };
}
...
module.exports = {
  ...
  /**
   * Envia texto via ManyChat.
   * Requer: settings.manychat_api_token (ou env) e subscriberId numérico.
   * Se cair fora da janela, usa flow de fallback (settings.manychat_fallback_flow_id ou env).
   */
  async sendText({ subscriberId, text }, settings = {}) {
    ...
    try {
      // ✅ sempre use o namespace /fb para enviar conteúdo (inclusive no WhatsApp)
