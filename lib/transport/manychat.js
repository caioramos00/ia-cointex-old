...
async function sendText({ subscriberId, text }, settings) {
  ...
  const r = await call('/whatsapp/sending/sendContent', payload, token, 'sendContent:text');
...
async function sendImage({ subscriberId, imageUrl, caption }, settings, opts = {}) {
  ...
  const imgMsg = { type: 'image', url: finalUrl };
  ...
  const r1 = await call('/whatsapp/sending/sendContent', payloadImg, token, 'sendContent:image');
  ...
  if (shouldAlsoSendAsFile) {
    ...
    await call('/whatsapp/sending/sendContent', payloadFile, token, 'sendContent:document-fallback');
  }
