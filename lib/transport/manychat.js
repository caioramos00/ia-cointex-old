 ...
 * ============================
 * - Preencha API_TOKEN com seu token do ManyChat (SEM "Bearer ").
 * - Se quiser forçar o fallback para enviar como documento junto da imagem,
 *   mude FORCE_FILE_FALLBACK para true.
 * - SILENT = false para ver logs detalhados.
 */
const API_TOKEN = '';           // <-- COLE AQUI seu token ManyChat
const FALLBACK_FLOW_ID = '';    // opcional: Flow ID p/ testes (não usado aqui)
const FORCE_FILE_FALLBACK = false; // true => sempre envia também como "file/document"
const SILENT = false;

// Limite de segurança (WhatsApp costuma barrar > ~5–10MB). Aqui apenas logamos alerta:
...
function resolveToken() {
  const tok = sanitizeToken(API_TOKEN);
  return tok;
}

function resolveFlowId() {
  return String(FALLBACK_FLOW_ID || '').trim();
}
...
// Faz HEAD pra checar Content-Type e tamanho — bom pra diagnosticar host/CDN
...
async function sendText({ subscriberId, text }) {
...
async function sendImage({ subscriberId, imageUrl, caption }, _settingsIgnored, opts = {}) {
  // _settingsIgnored mantido apenas para compatibilidade com chamadas existentes
  const token = resolveToken();
  if (!token) throw new Error('ManyChat API token ausente');

  // 1) diagnóstico do host/CDN
  const info = await headForInfo(imageUrl).catch(() => ({ ok: false, ct: '', len: 0 }));
  if (info.len > MAX_IMAGE_BYTES) {
    console.warn(`[ManyChat][sendImage] Arquivo possivelmente grande (${info.len} bytes). WhatsApp pode recusar > ~5–10MB.`);
  }

  const looksImage = isLikelyImageUrl(imageUrl, info.ct);
  if (!looksImage) {
    console.warn(`[ManyChat][sendImage] Content-Type inesperado ("${info.ct}"). Vou tentar mesmo assim.`);
  }

  // 2) cache-busting p/ evitar "mídia repetida"
  const finalUrl = addCacheBust(imageUrl);

  // 3) filename ajuda alguns clientes a renderizar corretamente
  ...
  // 5) fallback/duplo como ARQUIVO (document) – útil quando o preview inline some
...
async function sendDocument({ subscriberId, fileUrl, filename }) {
  const token = resolveToken();
  if (!token) throw new Error('ManyChat API token ausente');
